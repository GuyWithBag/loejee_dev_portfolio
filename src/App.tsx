...
const App: FC = () => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (key === "d" || key === "arrowright") {
        ...
        newIndex = (currentIndex - 1 + navigablesRef.current.length) % navigablesRef.current.length;
      } else if (key === "s" || key === "arrowdown") {
        newIndex = Math.min(currentIndex + 2, navigablesRef.current.length - 1);
  ...
  const renderModalContent = () => {
    ...
    const modalData: Record<string, any> = {
      hero: {
        theme: "bg-purple-500 dark:bg-purple-600 text-white border-purple-400 dark:border-purple-500",
        content: (
          <>
      ...
      about: {
        theme: "bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 border-slate-200 dark:border-slate-800",
        content: (
          <>
            ...
            <div className="relative z-10">
              ...
              {BIO.about.map((p, i) => (
                <p key={i} className="text-slate-500 dark:text-slate-400 text-lg mb-4">{p}</p>
              ))}
              <p className="text-slate-500 dark:text-slate-400 text-lg italic">Approach: {BIO.approach}</p>
            </div>
          </>
        )
      },
          ...
          <>
            ...
            <div className="relative z-10">
              ...
              <ul className="space-y-4 max-h-[400px] overflow-y-auto pr-4 no-scrollbar">
                {SKILLS.map((skill, i) => (
                  <li key={i} className="flex items-center gap-6 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700/50 shadow-sm">
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        <div className="absolute inset-0 bg-slate-900/40 dark:bg-black/60 backdrop-blur-sm" onClick={() => setActiveModal(null)}></div>
        <div className={`relative w-full max-w-2xl rounded-[2.5rem] p-10 shadow-3xl border overflow-hidden transition-all duration-300 ${data.theme}`}>
          <button 
  ...
  return (
    <div className={`p-4 md:p-10 min-h-screen flex flex-col items-center font-sans text-slate-800 dark:text-slate-100 ${isOnboarding ? "onboarding-active" : ""}`} onClick={() => endOnboarding()}>
      {/* Dark Mode Toggle */}
      <button 
      ...
      {isOnboarding && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 bg-slate-900/90 dark:bg-white/90 text-white dark:text-slate-900 px-6 py-3 rounded-full backdrop-blur-md font-bold text-sm z-[60] flex items-center gap-3 animate-bounce shadow-2xl border border-white/10">
          <div className="flex gap-1">
            {["W", "A", "S", "D"].map(key => (
              <kbd key={key} className="px-2 py-1 bg-slate-800 dark:bg-slate-200 rounded text-xs">{key}</kbd>
            ))}
          </div>
      ...
      <div className="max-w-5xl w-full flex flex-col gap-6 mt-10 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 auto-rows-[180px] w-full">
          ...
          <div
            id="hero-card"
            tabIndex={0}
            ref={el => { navigablesRef.current[0] = el; }}
          ...
          <div
            tabIndex={0}
            ref={el => { navigablesRef.current[1] = el; }}
            onMouseMove={handleMouseMove}
            onMouseLeave={handleMouseLeave}
          ...
          <div
            tabIndex={0}
            ref={el => { navigablesRef.current[2] = el; }}
            onMouseMove={handleMouseMove}
            onMouseLeave={handleMouseLeave}
          ...
          >
            ...
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1 z-10 pointer-events-none">Building from</p>
            <p className="font-semibold text-slate-800 dark:text-slate-200 z-10 pointer-events-none">{BIO.location}</p>
          </div>
          ...
          <a
            ...
            tabIndex={0}
            ref={el => { navigablesRef.current[3] = el; }}
            onMouseMove={handleMouseMove}
            onMouseLeave={handleMouseLeave}
          ...
          <div
            tabIndex={0}
            ref={el => { navigablesRef.current[4] = el; }}
            onMouseMove={handleMouseMove}
            onMouseLeave={handleMouseLeave}
            ...
            <div className="z-10 relative pointer-events-none">
              ...
              <div className="flex flex-wrap gap-3">
                {SKILLS.slice(0, 4).map((skill, i) => (
                    <span key={i} className="px-4 py-2 bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 font-semibold rounded-full text-sm flex items-center gap-2 shadow-sm border border-blue-200 dark:border-blue-800">
          ...
          <a
            ...
            rel="noopener noreferrer"
            tabIndex={0}
            ref={el => { navigablesRef.current[5] = el; }}
            onMouseMove={handleMouseMove}
            onMouseLeave={handleMouseLeave}
          ...
          <button
            tabIndex={0}
            ref={el => { navigablesRef.current[6] = el; }}
            onMouseMove={handleMouseMove}
            onMouseLeave={handleMouseLeave}
        ...
        <div className="flex flex-wrap items-start justify-between gap-6 mt-6 blur-target pb-20 w-full" id="projects-container">
          {PROJECTS.map((project, index) => {
            ...
            const navigableIndex = 7 + index;

            return (
              <div
                key={project.id}
                tabIndex={0}
                ref={el => { navigablesRef.current[navigableIndex] = el; }}
                onMouseMove={handleMouseMove}
                onMouseLeave={handleMouseLeave}
                onClick={() => toggleProject(project.id)}
                className={`navigable project-card grow rounded-[2.5rem] text-white shadow-2xl cursor-pointer relative overflow-hidden group border border-slate-200 dark:border-slate-800 flex flex-col 
                  ${isExpanded ? "project-expanded" : "basis-[300px]"}
                  ${project.aspectRatio === '16:9' ? 'aspect-video' : project.aspectRatio === '9:16' ? 'aspect-[9/16]' : 'aspect-square'}
                `}
              >
                {/* Expanded Content */}
                <div className={`${isExpanded ? "flex" : "hidden"} w-full md:w-1/2 p-8 md:p-12 flex-col justify-between h-full bg-slate-900 dark:bg-slate-950 z-30 overflow-y-auto`}>
                  <div>
                    ...
                    <p className="text-slate-300 font-medium leading-relaxed text-lg mb-6">{project.description}</p>
                    <div className="flex flex-wrap gap-2 mb-8">
                      {project.skills.map((skill, i) => (
                        <span key={i} className="px-3 py-1 bg-blue-500/20 text-blue-300 rounded-full text-sm font-bold border border-blue-500/30 flex items-center gap-1">
                          {skill}
                        </span>
                      ))}
                    </div>
                ...
                <div className="media-container relative w-full h-full flex-1 min-h-[300px] overflow-hidden pointer-events-none group-hover:pointer-events-auto">
                  <div className={`absolute inset-0 bg-slate-900/50 z-10 default-overlay transition-opacity duration-300 pointer-events-none ${isExpanded ? "opacity-0" : ""}`}></div>
                  <div 
                    className="carousel-track w-full h-full flex transition-transform duration-500 pointer-events-none"
                    style={{ transform: `translateX(-${carouselIndex * 100}%)` }}
                  >
                    {project.images.map((img, i) => (
                      <img key={i} src={img} className="w-full h-full object-cover shrink-0" alt={`${project.title} UI ${i+1}`} />
                    ))}
                  </div>
                  
                  {/* Carousel Controls */}
                  <div className={`${isExpanded ? "flex" : "hidden"} absolute inset-0 z-20 pointer-events-none p-4 justify-between items-center`}>
                    <button 
                      onClick={(e) => { e.stopPropagation(); moveCarousel(project.id, -1, project.images.length); }}
                  ...
                  <div className={`${isExpanded ? "hidden" : "flex"} absolute inset-0 p-8 flex flex-col justify-between z-20 pointer-events-none`}>
                    <div>
                    ...
                    <div className="flex gap-2">
                      {project.skills.slice(0, 2).map((skill, i) => (
                        <span key={i} className="px-4 py-2 bg-white/20 rounded-full text-sm font-bold backdrop-blur-md border border-white/30">{skill}</span>
                      ))}
                    </div>
                  </div>
                </div>
